...
namespace AlJohary.ServiceHub.Infrastructure.Persistence
{
    public class ReportRepository : IReportRepository
    {
        ...
        // SUPERSEDED for the reports area by GetFinancialOperations below. This older log mixes
        // maintenance rows by repair_orders.delivery_date and shows a lump paid_amount, so its
        // numbers do NOT reconcile with the payment-date-based KPI summary. Kept only for back-
        // compat; the daily/monthly operations pages use GetFinancialOperations instead.
        ...
        // Audit-safe financial operations log. One UNION ALL over the 7 money-affecting sources,
        // each recognized by its own real transaction/payment date (not delivery_date). Columns are
        // identical across all sources so the result binds to a single dynamic grid:
        //   Date, OperationType, Reference, Details, PaymentMethod, MoneyIn, MoneyOut, Deduction,
        //   NetEffect, UserName.
        // وارد = money in (NetEffect +), صادر = money out (NetEffect -), خصم/تسوية = employee
        // deduction (cost reducer, NetEffect + but NOT cash). Per-method MoneyIn/MoneyOut totals
        // reconcile with the KPI summary's payment_inflows/payment_outflows by construction (same
        // source rows, same date columns). Supplier 'purchase' rows are excluded (never cash out).
